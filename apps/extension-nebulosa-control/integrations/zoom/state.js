...
function classifySurface(urlString = window.location.href) {
  ...
  if (/^\/wc\/.+\/join/i.test(path)) return 'wc_join';
  if (/^\/wc\//i.test(path)) return 'wc_meeting';
...
function detectMeetingReadiness() {
  ...
  if (surface === 'wc_join') {
    return { surface, meetingState: MeetingState.JOINING, reason: 'join_page_detected' };
  }
  ...
  const participantPanel = _queryFirst(ZoomSelectors.PARTICIPANTS_PANEL);
  const participantRows = participantPanel
    ? _queryAll(ZoomSelectors.PARTICIPANT_ROW, participantPanel)
    : _queryAll(ZoomSelectors.PARTICIPANT_ROW);
  const toolbar = _queryFirst(ZoomSelectors.TOOLBAR);

  if (!participantPanel || !toolbar) {
    return {
      surface,
      meetingState: MeetingState.IN_MEETING_DOM_NOT_READY,
      reason: !participantPanel ? 'participant_panel_missing' : 'toolbar_missing',
...
function detectCapabilities() {
  ...
  const cameraTiles = _queryAll(ZoomSelectors.VIDEO_TILE);
